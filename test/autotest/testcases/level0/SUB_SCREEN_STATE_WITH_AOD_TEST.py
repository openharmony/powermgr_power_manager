...
class SUB_SCREEN_STATE_WITH_AOD_TEST(TestCase):
    ...
    def setup(self):
        self.log.info("SUB_SCREEN_STATE_WITH_AOD_TEST start")
        #处理可能会弹出的USB连接方式弹窗
    ...
    def test_step(self):
        ...
        device_logger.start_log(get_report_dir() + '//screen_state_with_aod.txt')
        self.driver.wait(30)
        ...
        CheckPoint("查看inactive日志")

        log_check_result = device_logger.check_log("Transit from DIM to INACTIVE for TIMEOUT", EXCEPTION=True)
        self.driver.Assert.equal(log_check_result, True)

    def teardown(self):
        Step("收尾工作")
        self.log.info("SUB_SCREEN_STATE_WITH_AOD_TEST down")
