...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    g_serviceTest.TestPowerServiceStub(static_cast<uint32_t>(
        IPowerMgrIpcCode::COMMAND_REGISTER_SHUTDOWN_CALLBACK_IPC_IN_ISYNCSHUTDOWNCALLBACK_IN_INT), data, size);
    g_serviceTest.TestPowerServiceStub(static_cast<uint32_t>(
        IPowerMgrIpcCode::COMMAND_UN_REGISTER_SHUTDOWN_CALLBACK_IPC_IN_ISYNCSHUTDOWNCALLBACK), data, size);
    return 0;
}
