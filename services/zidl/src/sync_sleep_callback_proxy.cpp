...
namespace OHOS {
namespace PowerMgr {
void SyncSleepCallbackProxy::OnSyncSleep(bool onForceSleep)
{
    ...
    if (!data.WriteInterfaceToken(SyncSleepCallbackProxy::GetDescriptor())) {
...
void SyncSleepCallbackProxy::OnSyncWakeup(bool onForceSleep)
{
    sptr<IRemoteObject> remote = Remote();
    RETURN_IF(remote == nullptr);
