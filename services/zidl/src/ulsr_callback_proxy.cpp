...
namespace OHOS {
namespace PowerMgr {
void UlsrCallbackProxy::OnSyncUlsr()
{
    ...
    if (!data.WriteInterfaceToken(UlsrCallbackProxy::GetDescriptor())) {
        POWER_HILOGE(FEATURE_SUSPEND, "Write descriptor failed");
        return;
    }

    int ret = remote->SendRequest(
        static_cast<int>(PowerMgr::UlsrCallbackInterfaceCode::CMD_ON_SYNC_ULSR),
        data, reply, option);
    if (ret != ERR_OK) {
...
void UlsrCallbackProxy::OnAsyncWakeup(bool ulsrResult)
{
    sptr<IRemoteObject> remote = Remote();
    RETURN_IF(remote == nullptr);
    ...
    RETURN_IF_WRITE_PARCEL_FAILED_NO_RET(data, Bool, ulsrResult);

    int ret = remote->SendRequest(
        static_cast<int>(PowerMgr::UlsrCallbackInterfaceCode::CMD_ON_ASYNC_WAKEUP),
        data, reply, option);
    if (ret != ERR_OK) {
